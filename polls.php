	...
	case "delete":
		if(get_user_class() >= UC_MODERATOR){
			if(isset($_GET['sure']) && $_GET['sure'] == 1){
				...
				stderr("Umfrage löschen", "Willst Du die Umfrage löschen? Klicke\n" . "<a href=\"" . $_SERVER['SCRIPT_NAME'] . "?action=delete&pollid=" . $_GET['pollid'] . "&sure=1\">hier</a>, wenn Du Dir sicher bist.");
		}else
			stderr("Error", "Du hast dazu keine Berechtigung!");
		break;
	case "wipevotes":
		if(get_user_class() >= UC_MODERATOR){
			if(isset($_GET['sure']) && $_GET['sure'] == 1){
				...
				stderr("Votes löschen", "Willst Du das Abstimmungsergebnis löschen? Klicke\n" . "<a href=\"" . $_SERVER['SCRIPT_NAME'] . "?action=wipevotes&pollid=" . $_GET['pollid'] . "&sure=1\">hier</a>, wenn Du Dir sicher bist.");
		}else
			stderr("Error", "Du hast dazu keine Berechtigung!");
		break;
	case "cheat":
...
if($action == "view"){
	begin_frame("Umfragen - <a href=\"" . $_SERVER['SCRIPT_NAME'] . "?action=create\">[Neue Umfrage]</a>", FALSE, "100%");
	begin_table(TRUE);
	...
	foreach($all_polls as $poll){
		...
		foreach($poll['answers'] as $k => $a){
			...
			echo "                    <p><b>" . $a . " - " . $c . " Stimmen - " . $p . "%</b> <a href=\"" . $_SERVER['SCRIPT_NAME'] . "?action=cheat&pollid=" . $poll['id'] . "&aid=" . $k . "\"><img src=\"".$GLOBALS["PIC_BASE_URL"].$GLOBALS["ss_uri"]."/plus.gif\"></a></p>\n";
		}
		echo "                    </td>".
			"                </tr>";
		if (get_user_class() >= UC_MODERATOR){
			echo "                <tr>\n".
				"                    <td width=\"100%\" class=\"tablea\">\n".
				"                    <p><b>Optionen:<br>".
				"                        <a href=\"" . $_SERVER['SCRIPT_NAME'] . "?action=delete&pollid=" . $poll['id'] . "\">Diese Umfrage löschen</a><br>\n".
				"                        <a href=\"" . $_SERVER['SCRIPT_NAME'] . "?action=wipevotes&pollid=" . $poll['id'] . "\">Alle Antworten löschen</a>\n".
				"                    </b></p>\n".
				"                    </td>\n".
				"                </tr>\n";
		}
	...
	echo "<script language=\"JavaScript\" src=\"js/addInput.js\" type=\"text/javascript\"></script>\n".
		"<form method=\"post\" action=\"" . $_SERVER['SCRIPT_NAME'] . "?action=create\">\n".
		"<table cellpadding=\"4\" cellspacing=\"1\" border=\"0\" style=\"width:100%\" class=\"tableinborder\">\n".
		"    <tr class=\"tabletitle\" width=\"100%\">\n".
